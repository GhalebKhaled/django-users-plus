...
class BaseUserAdmin(django.contrib.auth.admin.UserAdmin):
    form = UserChangeForm
    ...
    def get_readonly_fields(self, request, obj=None):
        readonly_fields = super(BaseUserAdmin, self).get_readonly_fields(
            request, obj)
        if not request.user.is_superuser:
    ...
    def log_addition(self, request, object, message):
        super(BaseUserAdmin, self).log_addition(request, object, message)
    ...
    def change_view(self, request, object_id, form_url='', extra_context=None):
        ...
        response = super(BaseUserAdmin, self).change_view(request, object_id, form_url, extra_context)

        if request.method == 'POST':
            # be sure to refresh the user since its likely changed
            user = apps.get_model(settings.AUTH_USER_MODEL).objects.get(pk=object_id)
            if user.email != old_email:
                signals.user_email_change.send(sender=self.__class__, request=request, user=user, old_email=old_email,
                    new_email=user.email)
            if user.is_active != old_is_active:
                if user.is_active:
                    signals.user_activate.send(sender=self.__class__, request=request, user=user)
                else:
                    signals.user_deactivate.send(sender=self.__class__, request=request, user=user)
        return response
    ...
    def reset_user_password(self, request, user_id):
        ...
        form = django.contrib.auth.forms.PasswordResetForm(data={'email': user.email, })
        form.is_valid()

        form.save()
        self.message_user(request, 'Password reset email sent to {0}.'.format(user.email))

        signals.user_password_reset_request.send(sender=self.__class__, request=request, user=user)
    ...
    def user_change_password(self, request, id, form_url=''):
        if not self.has_change_permission(request):
            raise django.core.exceptions.PermissionDenied
        user = django.shortcuts.get_object_or_404(self.get_queryset(request), pk=id)
        if request.method == 'POST':
        ...
        context = {
            'title': _('Change password: %s') % django.utils.html.escape(user.get_username()),
            'adminForm': adminForm,
            'form_url': form_url,
            'form': form,
            'is_popup':
...
class BaseCompanyAdmin(django.contrib.admin.ModelAdmin):
    ...
    fields = ('created_on', 'updated_on', 'name', 'street_address', 'street_address_2', 'city', 'state', 'postal_code', )
    ...
    def save_model(self, request, obj, form, change):
        if 'name' in form.changed_data:
            signals.company_name_change.send(
                sender=self.__class__, request=request, user=request.user, company=obj, old_name=form.initial.get('name'),
                new_name=obj.name)
        super(BaseCompanyAdmin, self).save_model(request, obj, form, change)


class BaseAuditLogEventAdmin(django.contrib.admin.ModelAdmin):
    list_filter = ('company_name', )
    search_fields = ('user_email', 'message', )
    list_display = ('recorded_on', 'user', 'company', 'is_masquerading', 'masquerading_user', 'message', )
    ordering = ('-recorded_on', )
    list_per_page = 50
    fieldsets = (
        ('', {
            'fields': ('recorded_on', 'user_id', 'user_email', 'company_name', 'company_id', ),
        }),
        ('Masquerading User', {
    ...
    readonly_fields = ('created_on', 'updated_on', 'recorded_on', 'user_id', 'user_email', 'company_name', 'company_id',
                       'message', 'masquerading_user_email', 'masquerading_user_id', )
    actions = []
    ...
    def company(self, obj):
        return '{}&nbsp;({})'.format(obj.company_name, obj.company_id)
    user.admin_order_field = 'company_id'
    user.allow_tags = True

    def masquerading_user(self, obj):
        if obj.is_masquerading:
            return '{}&nbsp;({})'.format(obj.masquerading_user_email, obj.masquerading_user_id)
        else:
            return ''
